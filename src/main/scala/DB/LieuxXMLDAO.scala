...
object LieuxXMLDAO {
  ...
  /** @param id l'id d'une organisation valide
  ...
  def getAdresseId(id: String): Option[String] = {
    ...
    val query: String =
      s"SELECT street_number,street_name FROM addresses WHERE organization_id= '$id'"
  ...
  /** @param mots Liste non-vide de mot a rechercher dans les name d'organisation
    * @return retourn l'ensemble des id d'organisation qui contiens les mots dans name , None si ensemble vide
    */
  def getId(mots: List[String]): Option[List[String]] = {
    val connection = DriverManager.getConnection("jdbc:sqlite:" + dbFilePath)
    val statement: Statement = connection.createStatement()
    var req: String =
      "SELECT id FROM organizations WHERE id IN (SELECT organization_id FROM addresses WHERE city = 'Rennes') AND "
