...
trait OutilsWeb {
   ...
   */
  def obtenirHtml(url: String): Html
   ...
   */
  def combinerUrls(base: String, local: String): String
}

/** L'objet parseur d'URL */

object OutilsWebObjet extends OutilsWeb {
  ...
  def obtenirHtml(url: String): Html = {
    try {
      ...
      // pour faire croire au site que l'on est un browser, on définit un User-Agent
      urlc.setRequestProperty("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.14; rv:84.0) ")
      val inputFile = urlc.getInputStream()
      val rootNode = cleaner.clean(inputFile)
      tag2Html(rootNode)
    } catch {
      case _: java.net.MalformedURLException   =>
        println("Malformed URL: " + url); throw TagNodeConversionException("Malformed URL")
      case _: java.net.UnknownServiceException =>
        println("Unknown Service on " + url); throw TagNodeConversionException("Unknown Service")
      case _: java.net.ConnectException        =>
        println("Connect Exception on " + url); throw TagNodeConversionException("Connect Exception")
      case _: java.io.IOException              =>
        println("IO Exception on " + url); throw TagNodeConversionException("IO Exception")
  ...
  private def tag2Html(tn: BaseToken): Html = {
    ...
    tn match {
      ...
      case ht: HtmlNode => Texte(tn.toString)
      case _            => Texte("")
    }
  }
